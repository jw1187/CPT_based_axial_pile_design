function [Q,Qf,Qp,Qt,Qz,Qzt,tau,qbz,S_prc,S_fc_prc,S_ft_prc]=Fugro_full_liq(z,qc,Ic,sgp,D,WT,L,sg,De)
...
y1(find(y1<4)) = 4;
y2 = ((ones(size(z))*L-z*ones(size(L)))); y2(find(y2<0)) = 0;   % for clay
...
sg_D = movmean(sg',K)';
if Ic(ind)<2.6
    Qp = 8.5.*(qc_D(ind)/101.3).^0.5.*(Ar^0.25)*pi*(D.^2)/4*101.3;
else
    Qp = 0.7*(qc_D(ind)-sg_D(ind))*pi*(D.^2)/4;
end
